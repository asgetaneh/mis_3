 ...
 @forelse(getReportingQuarter($planAcc->Kpi->reportingPeriodType->id) as $period)
    ...
    @endforeach
        ...
        @php
            ...
            $planSumch2Total = reportIndividualChTwoSum($planAcc->Kpi->id,  $two->id,$office,$period->id, 1);
            @endphp
            {{$planSumch2Total}}
            </td>
    ...
    <tr>
        ...
        @foreach ($planAcc->Kpi->kpiChildOnes as $one)
            ...
            @php
                ...
                $planSumch1ch3 = reportIndividualChOnechThreeSum($planAcc->Kpi->id, $one->id, $two->id,$kpiThree->id,$office, 1);
                @endphp
                    {{$planSumch1ch3}}
            </td>
        @endforeach
