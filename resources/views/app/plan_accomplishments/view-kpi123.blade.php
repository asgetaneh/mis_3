<table class="table table-bordered" style="background:#12cd4322;">
    @php
         $ospan = $planAccKpiChildThree->count() * ($getQuarter->count()+1) + 2;
    @endphp
    <!-- <tr id="child-ones"> -->
    <tr>
    ...
    <tr>
        ...
        <th colspan="{{ $planAccKpiChildThree->count() }}">Baseline</th>
    ...
    @forelse ($planAccKpiChildOne as $one)
        ...
        @foreach ($planAccKpiChildTwo as $two)
                ...
                @foreach ($planAccKpiChildThree as $three)
                    @php
                        $baselineOfOfficePlan  = planBaseline($planAccKpi->id,$office, $planning_year->id, $period->id,$one->id,$two->id,$three->id);
                    @endphp
                        ...
                        @foreach ($planAccKpiChildOne as $one) --}}
                            ...
                            @foreach ($planAccKpiChildThree as $kpiThree)
                                <td>
                                    ...
                                    // $plan123 = $planAcc->planIndividual($planAccKpi->id, $one->id, $two->id, $kpiThree->id, $office, $period->id,false);
                                        $plan123 = $planAcc->KpiOTT($planAccKpi->id, $office, $period->id,false,$planning_year->id ?? NULL ,$one->id, $two->id, $kpiThree->id);
    ...
    <tr>
        ...
        <td colspan="{{ ($getQuarter->count()+1) *$planAccKpiChildThree->count()+1}}">
 ...
 <div class="collapse" id="off{{ $office->id }}{{$planAccKpi->id}}">
    <div class="card card-body">
        ...
        @forelse ($offices_twos as $office)
            ...
            @include('app.plan_accomplishments.view-kpi1233')
            <div class="collapse" id="off{{ $office->id }}{{$planAccKpi->id}}">
                <div class="card card-body">
                    @php
