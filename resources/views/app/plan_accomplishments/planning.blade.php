                        ...
                        <div class="tab-content" id="custom-tabs-four-tabContent">
                            ...
                            @forelse($allData as $data)
                                                ...
                                                <thead>
                                                    <tr id="child-ones">
                                                        ...
                                                        @foreach ($kpi->kpiChildOnes as $one)
                                                            <th id="childer">{{ $one->kpiChildOneTranslations[0]->name }}
                                                            </th>
                                                        @endforeach
                                                    </tr>
                                                </thead>

                                                @foreach ($kpi->kpiChildOnes as $kpiOne)
                                                    {{-- If UG, PG have Child twos (Regular, Summer) --}}
                                                    @if (!$kpiOne->kpiChildTwos->isEmpty())
                                                        @foreach ($kpiOne->kpiChildTwos as $kpiTwo)
                                                            @if (!$kpiTwo->kpiChildThrees->isEmpty())
                                                                <script>
                                                                    var childerOne = document.getElementById('childer')
                                                                    for(let i = 0; i <= {{ $kpiTwo->kpiChildThrees->count() }}; i++){
                                                                        childerOne.colSpan = {{ $kpiTwo->kpiChildThrees->count() }};
                                                                    }

                                                                </script>
                                                                <tr>
                                                                    <td></td>
                                                                    @foreach ($kpiTwo->kpiChildThrees as $kpiThree)
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    @foreach ($kpi->kpiChildOnes as $kpiOne)
                                                                        <td>
                                                                            <input name="child1-child2-child3" class="form-control" type="number" required>
                                                                        </td>
                                                                    @endforeach
                                                            ...
                                                            @else
                                                                <tr>
                                                                    ...
                                                                    @foreach ($kpi->kpiChildOnes as $kpiOne)
                                                                        <td><input name="child1-child2" class="form-control" type="number" required></td>
                                                                    @endforeach
                                        ...
                                        @else
                                            <p class="mb-3">
                                                <input class="form-control" type="number" placeholder="Enter KPI value"
                                                    name="kpi-only" required>
                                            </p>
                                        @endif

                                        <textarea name="goal-obj-kpi-narration" style="height: 100px;" class="form-control" name="" id="" placeholder="Narration here" required></textarea>
                                ...
                                {{-- <table class="table table-bordered">
                                        ...
                                        <tr>
                                            <td
                                                class="{{ $kpi->reportingPeriodType->reportingPeriodTypeTs[0]->name == 'Quarterly' ? 'bg-primary' : 'bg-success' }} h5">
                                                KPI: {{ $kpi->KeyPeformanceIndicatorTs[0]->name }}
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                @if ($kpi->kpiChildOnes->isEmpty())
                                                    <input type="text" placeholder="Enter kpi value"
                                                        name="{{ 'obj' . $kpi->objective_id . 'kpi' . $kpi->id }}"
                                                        class="form-control">
                                                @else
                                                    <table class="table table-bordered">
                                                        <tr>

                                                            @forelse($kpi->kpiChildOnes as $kpiChildOne)
                                                                <thead>
                                                                    <tr>
                                                                        <th></th>
