                                        ...
                                        <div class="card p-2">
                                            ...
                                            @if (hasOfficeActiveReportComment(auth()->user()->offices[0]->id, $kpi_id, $planning_year->id ?? NULL)->count() > 0)
                                                <div class="bg-light w-5 float-right p-3">
                                                    <p class="m-auto">You have comment from <u>{{ getReportCommentorInfo(auth()->user()->offices[0]->id, $planAcc->kpi_id, $planning_year->id ?? NULL)->name ?? '-' }}</u>
                                                        <a  class="btn btn-sm btn-flat btn-info text-white view-comment"
                                                            data-toggle="modal" data-target="#view-comment-modal"
                                                            data-id="{{ getReportCommentorInfo(auth()->user()->offices[0]->id, $planAcc->kpi_id, $planning_year->id ?? NULL)->translation_id ?? '-' }}-{{$kpi_id}}-{{$planning_year->id ?? NULL}}">
