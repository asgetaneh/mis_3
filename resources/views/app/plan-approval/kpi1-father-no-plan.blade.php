<tr>
    ...
    @if(planStatusOfficeApproved($office, $planAcc->kpi_id, $planning_year[0]->id, $hasChildrenOfficesPlannedAndApproved) !== auth()->user()->offices[0]->level)
        <th class="bg-light">
...
@forelse(getQuarter($planAcc->Kpi->reportingPeriodType->id) as $period)
    <td>
        @php
            $planOne = planOneApproved($planAcc->Kpi->id, $one->id, $office, $period->id, $planning_year[0]->id);
