<table class ="table table-bordered">
    ...
    @if($first==1)
        ...
        @if(planStatusOffice($office, $planAcc->kpi_id, $planning_year->id ?? NULL, $hasChildrenOfficesPlannedAndApproved) !== auth()->user()->offices[0]->level)
            <th class="bg-light">
    ...
    <tr>
        ...
        @forelse(getQuarter($planAcc->Kpi->reportingPeriodType->id) as $period)
            @php
                $planOfOfficePlan
                = planSumApproved($planAcc->Kpi->id,$office, $period->id, $planning_year->id ?? NULL);
