    ...
    <th>Sum</th>
        ...
        @forelse(getQuarter($planAcc->Kpi->reportingPeriodType->id) as $period)
             @php
                ...
                $planKpiOfficeYear = planSumOfKpi($planAcc->Kpi->id, $office, 3);
            ...
            <tr>
                ...
                @foreach ($planAcc->Kpi->kpiChildTwos as $two)
                    ...
                    @foreach ($planAcc->Kpi->kpiChildOnes as $one)
                        <td>
                            @php
                                $planOneTwo = planOneTwo($planAcc->Kpi->id, $one->id, $two->id, $office, $period->id, 3);
                            @endphp
                            {{ $planOneTwo }}
                        </td>
                    @endforeach
                    ...
                    <td>
                        @php
                            ...
                            $planSumch2Total = planIndividualChTwoSum($planAcc->Kpi->id, $two->id, $office,$period->id, 3);
                        @endphp
                        {{ $planSumch2Total }}
                    </td>
                    {{-- end total ch2 --}}
        ...
        <tr>
            ...
            <td>
                @php
                    ...
                    $planSumch1ch3 = planIndividualChOnech($planAcc->Kpi->id, $one->id, $two->id, $office, 3);
                @endphp
                {{ $planSumch1ch3 }}
            </td>
            @endforeach
