        ...
        <tr>
            ...
            @foreach ($planAcc->Kpi->kpiChildTwos as $two)
                ...
                @foreach ($planAcc->Kpi->kpiChildOnes as $one)
                    <td>
                        @php
                            $planOneTwo = planOneTwo($planAcc->Kpi->id, $one->id, $two->id, auth()->user()->offices[0], $period->id, 7);
                        @endphp
                        {{ $planOneTwo }}
                    </td>
                @endforeach
    ...
    <tr>
        ...
        <td>
            @php
                ...
                $planSumch1ch3 = planIndividualChOnech($planAcc->Kpi->id, $one->id, $two->id, auth()->user()->offices[0], 7);
            @endphp
            {{ $planSumch1ch3 }}
        </td>
        @endforeach
